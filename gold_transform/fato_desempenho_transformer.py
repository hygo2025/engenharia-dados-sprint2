...
class FatoDesempenhoTransformer(BaseTransformer):
    ...
    def transform_data(self):
        ...
        df = df_round.merge(df_clube, left_on='clube', right_on='nome', how='left').rename(columns={'clube_id': 'clube_id'})
        df = df.merge(df_tempo, on=['ano', 'rodada'], how='left').rename(columns={'tempo_id': 'tempo_id'})

        df['desempenho_id'] = df.index + 1
        return df[['desempenho_id', 'clube_id', 'tempo_id', 'jogos', 'vitorias', 'empates', 'derrotas', 'gols_pro', 'gols_contra', 'saldo', 'pontos']]
